 ...
 * a non-exportable installation public key). No Android ID, BLE address, or stable
 * BLE-derived digest is transmitted.
 */
internal class AnonymousTokenManager(
    private val installationIdentity: AndroidInstallationIdentity,
    ...
    suspend fun ensureAnonymousToken(): Result<String> = runCatching {
        tokenMutex.withLock {
            val enrollment = enrollmentWithRecovery()
            val existing = prefs.anonymousToken
            if (existing.isNotEmpty() &&
                prefs.anonymousTokenInstallationKeyId == enrollment.installationKeyId
            ) return@withLock existing
            prefs.anonymousToken = ""
            prefs.anonymousTokenInstallationKeyId = ""
            val resp = apiClient.service.anonymousToken(enrollment)
    ...
    private fun enrollmentWithRecovery() = try {
        installationIdentity.enrollmentRequest()
