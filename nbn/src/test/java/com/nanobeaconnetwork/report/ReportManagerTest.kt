...
class ReportManagerTest {
    ...
    private suspend fun makeDao(): PendingReportDao {
        val dao = mock<PendingReportDao>()
        whenever(dao.deleteExpiredAndExhausted(any(), any())).doReturn(0)
        return dao
    }
    ...
    @Test fun `R01c server-dropped batch is cleared not retried`() = runTest {
        ...
        // (e.g. dropped for an unknown EID). The batch must not be resent.
        val dao = makeDao()
        whenever(dao.count()).doReturn(1)
        whenever(dao.fetchBatch(500)).doReturn(listOf(makeReport()))
    ...
    @Test fun `R08 successful report resets backoff to 1 second`() = runTest {
        ...
        val manager = ReportManager(dao, makeApiClient(), makeConfig(), this) { nowMs }
        manager.tryFlush()
        nowMs += 1_001
        manager.tryFlush()
    ...
    @Test fun `parallel flushes do not upload the same batch twice`() = runTest {
        val dao = makeDao()
        whenever(dao.count()).doReturn(0)
        whenever(dao.fetchBatch(500)).thenReturn(listOf(makeReport()), emptyList())
        mockServer.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody("""{"status":"ok","count":1}""")
    ...
    @Test fun `expired and exhausted records increment failed count`() = runTest {
        val dao = makeDao()
        whenever(dao.count()).doReturn(0)
        whenever(dao.fetchBatch(500)).doReturn(listOf(makeReport()))
        whenever(dao.deleteExpiredAndExhausted(any(), any())).thenReturn(0, 2)
        mockServer.enqueue(MockResponse().setResponseCode(500))
        val manager = ReportManager(dao, makeApiClient(), makeConfig(), this)

        manager.tryFlush()

        assertEquals(2, manager.stats.value.failedCount)
    }
}
